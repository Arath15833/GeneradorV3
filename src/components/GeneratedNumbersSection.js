...
const GeneratedNumbersSection = ({ generatedNumbers, confirmedNumbers }) => {
  ...
  const AVAILABLE_COLOR = 'bg-gray-100 text-gray-700'; // Color para números disponibles (blanco/gris claro)
  const RESERVED_COLOR = 'bg-orange-400 text-white'; // Color para números apartados (naranja)
  const CONFIRMED_COLOR = 'bg-emerald-500 text-white'; // Color para números confirmados/pagados (verde esmeralda)

  return (
    <div className="p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 w-full max-w-6xl mx-auto">
      <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 dark:text-white mb-4 sm:mb-6 text-center">Estado de los Números</h2>
      
      {/* Simbología */}
      <div className="flex flex-wrap justify-center gap-4 mb-6 text-sm sm:text-base text-gray-800 dark:text-white">
        <div className="flex items-center">
          <span className={`w-4 h-4 rounded-full mr-2 ${CONFIRMED_COLOR}`}></span>
          <span>Verde - Comprado</span>
        </div>
        <div className="flex items-center">
