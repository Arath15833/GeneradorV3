...
const TicketCard = ({ ticket, onSaveTicket }) => {
  // Se elimina la función handleDownloadTicket

  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-4 sm:p-6 w-full max-w-sm mx-auto transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-4">
        <h3 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">{ticket.lotteryName}</h3>
        <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-full mt-1 sm:mt-0">
      ...
      <div className="mb-3 sm:mb-4">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {ticket.numbers.split(', ').map((num, index) => (
            <span
              key={index}
              className="w-20 h-10 sm:w-24 sm:h-12 flex items-center justify-center bg-purple-100 dark:bg-purple-700 text-purple-800 dark:text-purple-100 text-lg sm:text-xl font-bold rounded-lg shadow-inner"
            >
              {num}
            </span>
          ))}
      ...
      {/* Se elimina el botón de Descargar Boleto */}
    </div>
  );
};
