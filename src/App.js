...
const App = () => {
  ...
  const [currentPage, setCurrentPage] = useState('form'); // 'form', 'tickets', 'numbers', 'saved'
      ...
      case 'saved':
        return (
          <div className="p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 w-full max-w-4xl mx-auto">
            <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 dark:text-white mb-4 sm:mb-6 text-center">Mis Boletos Guardados</h2>
  ...
  const buttonClass = (page) => 
    `px-4 py-2 sm:px-6 sm:py-3 rounded-xl font-semibold text-base sm:text-lg transition-all duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-purple-300 ${
      currentPage === page
        ? 'bg-purple-600 text-white shadow-lg shadow-purple-400/50'
        : 'bg-white dark:bg-gray-700 text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-600 shadow-md hover:shadow-lg'
    }`;

  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-purple-100 dark:from-gray-900 dark:to-gray-800 text-gray-900 dark:text-white p-4 sm:p-8 font-sans">
